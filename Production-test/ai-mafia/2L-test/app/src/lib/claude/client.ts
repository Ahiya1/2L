...
const loadApiKey = (): string => {
  ...
  if (fs.existsSync('.anthropic-key.txt')) {
...
export function generateFallbackResponse(playerName: string): string {
  ...
  return fallbacks[randomIndex] || fallbacks[0] || `${playerName} remains silent.`;
}
...
export async function generateAgentResponse(
  ...
  for (let attempt = 1; attempt <= fullConfig.maxRetries; attempt++) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      // Log error details
      claudeLogger.error(
        { attempt, maxRetries: fullConfig.maxRetries, error: error.message, status: error.status },
      ...
      if (error.status === 429) {
        ...
        claudeLogger.warn({ delay }, 'Rate limited, retrying');
        await sleep(delay);
        continue;
      }
      ...
      if (error.status >= 500) {
        ...
        claudeLogger.warn({ delay, status: error.status }, 'Server error, retrying');
        await sleep(delay);
        continue;
      }
...
export async function generateWithTimeout(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (error.message === 'Agent response timeout') {
      claudeLogger.warn(
        { playerName: context.player.name, timeoutMs: fullConfig.timeoutMs },
...
export async function generateValidResponse(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    claudeLogger.warn(
      { attempt, maxAttempts, error: validation.error, response: response.text.substring(0, 100) },
