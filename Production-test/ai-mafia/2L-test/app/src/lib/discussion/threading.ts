...
import type { DiscussionMessage, Player } from '../types/shared';

// Type for messages with player relation included
type MessageWithPlayer = DiscussionMessage & { player: Player };

/**
 * Determine if a message should be threaded to another message
 *
 * @param message The message text to analyze
 * @param recentMessages Last 5-10 messages for context (must include player relation)
 * @returns Message ID to reply to, or null
 */
export function determineReplyTarget(
  message: string,
  recentMessages: MessageWithPlayer[]
): string | null {
  ...
  return recentMessages.length > 0 ? recentMessages[0]?.id ?? null : null;
}
...
export function extractAccusationTargets(message: string): string[] {
  ...
  for (const pattern of accusationPatterns) {
    ...
    for (const match of matches) {
      ...
      if (agentName && !targets.includes(agentName)) {
